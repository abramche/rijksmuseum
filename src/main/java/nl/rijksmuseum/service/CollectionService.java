...
public class CollectionService {
    ...
    public Collection getCollection(Culture culture, CollectionQuery query) {
        GenericHTTPResponse response = client.makeAuthenticatedRequest(culture, endpoint, query.toQueryString());
        return (Collection) deserializeResponse(response, Collection.class);
    }

    private Object deserializeResponse(GenericHTTPResponse response, Class<?> clazz) {
        try {
            return objectMapper.readValue(response.getResponseBody(), clazz);
        } catch (JsonProcessingException e) {
            throw new IllegalArgumentException("Unable to deserialize response: ", e);
        }
    }
}
